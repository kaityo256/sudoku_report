...
そんなことがあったことなどしばらく忘れていたのだが、2013年になって、私が所属するプロジェクトで「レプリカ交換モンテカルロ法」の効率が話題となった。詳細は後述するが、これは東大の福島さんたちが考案した手法で、普通のモンテカルロ法に比べて非常に効率がよくなる手法だ。特にスパコンでの並列計算と相性が良いことから、HPC(High Performance Computing)分野で広く使われていた。しかし、これがどういう問題で、どういう時に効率がよくなるのか、当時の私はよくわかっていなかった。その時、突然3年前の「世界一難しい数独」のことを思い出した。
...
その後の騒動は前述の通りである。それまでもプチ炎上はあったが、ここまでの炎上は初めての経験である。「ねとらぼ」「スラッシュドット」、自分が普段良く見るサイトに並ぶ自分の名前。私は「自分は心が強いほうだ」と思っているが、「アホか」「自分で解いてみなかったのか？」「東大暇そうだな」などのコメントが並ぶのを見て心が折れ、その日は職場を早退したのだった。
...
2016年5月30日、神保町にある書店「書泉グランデ」で、数独のイベントが行われ、私はそこに呼ばれた。正確には数独のイベントではなく、岩波データサイエンスという本の出版イベントで、イベント名も「[ナンプレと魔方陣　解いたり作ったり数えたり](https://www.shosen.co.jp/event/33704/)」というタイトルであった。最初に数独世界一の方による数独のリアルタイムチャレンジのあと、私の話をした。
...
[Kさんによる記事1](http://k.osask.jp/wiki/?sdk0001)
[Kさんによる記事2](http://k.osask.jp/wiki/?sdk0002)
[Kさんによる記事3](http://k.osask.jp/wiki/?sdk0003)
[Kさんによる記事4](http://k.osask.jp/wiki/?sdk0004)

## Aさんの参加

プロジェクトが停滞を見せていたころ、未踏PMでもあるTさんから、新たなメンバーを二人紹介していただいた。そのうちの一人、Aさんは強烈にすごかった。未踏同期のKさんもすごいのだが、それとは別のベクトルですごかった。何がすごいかというと、情報の検索能力とプログラミング能力が抜群だった。
...
数独プロジェクトが立ち上がってから長い間、僕は交換モンテカルロ法を使っていた。ここで、モンテカルロ法、および交換モンテカルロ法についてまとめておきたい。

![ナイーブなモンテカルロ法「ダーツ法」](img/darts.png)

「モンテカルロ法」というと、正方形の中に点をランダムに発生させ、四半円の中に点が入っているかどうかを判定することで円周率を計算する、という「ダーツ法」課題を思い出す人が多いかもしれない。これは、$0 < x,y < 1$を満たすような一様乱数$x$と$y$を$m$個発生させ、$x^2 + y^2< 1$となる回数$n$を数えると、その比$n/m$が$\pi/4$になる、ということを使って円周率が求まる、という仕組みだ。一様乱数の発生も、自乗の和も簡単に計算できるわりに、無理数(超越数)である円周率を求めることができるので、モンテカルロ法の例としてよく取り上げられるのだが、この方法は収束が遅いために実用に向かない。

一般に、数値計算手法として使われる「モンテカルロ法」とは、ほぼ「マルコフ連鎖モンテカルロ法(Markov-Chain Monte Carlo Method)」を指す。略してMCMCなどと呼ばれることもある。これはナイーブなモンテカルロ法と違い、極めて収束が早い。特に超多次元空間における数値積分に非常に有効であるため、統計力学の分野で広く使われている。

ナイーブなモンテカルロ法とマルコフ連鎖モンテカルロ法の違いは、「現在の状態」を持つかどうかにある。先程のダーツ法では、先に投げたダーツと次に投げるダーツは無関係だった。しかし、マルコフ連鎖モンテカルロは、「現在地」の概念があり、その近傍を選んでどこに移動するかどうかを選択する。いくつかの流儀があるが、広く使われるメトロポリス法では「まず移動する場所(trial)をランダムに決める」「決めた場所に移動するかどうか決める」ことを繰り返すことで空間を確率的に移動していく。

![マルコフ連鎖モンテカルロ法](img/mcmc.png)

こうして空間の中をランダムウォークしていくのだが、この際、「決めた場所(trial)」に、「移動する(accept)」か、「元の場所にとどまる(reject)」かの確率を上手く決めることで、マルコフ連鎖モンテカルロ法は極めて高効率に空間をサンプリングできるのだが、詳細は述べない。とりあえず数値計算界隈、特に統計力学方面では「モンテカルロといえばマルコフ連鎖モンテカルロ法を指し、それは一般に非常に効率が良い」と思われていることを知っておいていただければ良い。以後、単に「モンテカルロ法」と言ったら「マルコフ連鎖モンテカルロ法」のことを指す。

### 交換モンテカルロ法

モンテカルロ法にはエネルギーと温度の概念がある。モンテカルロ法で酔歩する人は、基本的にエネルギーが下がる向きに進みたがるのだが、たまにエネルギーが高い法にも進む。この「どれだけエネルギーが高い向きにも進もうとするか」を決めるパラメータが温度だ。高温であるほどエネルギー差を無視して進み、低温であるほどエネルギーが低い方へ低い方へ進むようになる。

僕はこの性質を使って、モンテカルロ法で数独の難問を探索することを考えた。
僕が考えた方法はこうだ。まず、数独の難易度を「エネルギー」だと思うことにしよう。難しければ難しいほど「エネルギーが低い」ように符号を取る。そして、モンテカルロ法により、数独のヒントを追加したり、減らしたりすることで問題を少しずつ修正していく。こうして「広大な数独の問題空間」の中を、モンテカルロ法により探索し、エネルギーが非常に低い場所(＝難問)を探すことを目指す。

基本的には難しくなる方向に持っていきたいが、それだけではヒントを減らすことしかできず、そのうち行き詰まるであろう。そこで、たまにはヒントを追加することも考えなければならない。温度を高くすればエネルギーが高いところも探索できるようになるが、高くしすぎる地形を無視して進んでしまうために純粋な酔歩になり、効率的に探索することができない。

逆に温度を低くしすぎるとエネルギーの低い方へ低い方へ進むので、「エネルギーの地形の盆地」にハマり込んでしまい、「ちょっと頑張って登れば到達できたはずの、もっとエネルギーが低い場所」に到達できなくなる。これを「ローカルミニマムにトラップされる」と呼ぶ。

![温度の効果](img/temperature.png)

このような問題を解決する方法として提案されたのが「レプリカ交換モンテカルロ法(Replica-Exchange Monte Carlo Method, REMC)」である。以後、交換モンテカルロ法と略す。

交換モンテカルロ法は、温度が異なる「レプリカ」を多数用意する。温度が低いレプリカは粒度の細かいていねいな探索を、温度が高いレプリカは粒度は粗いが広い範囲を探索する。イメージとしては「温度が高いレプリカ」が探索隊となり、「温度が低いレプリカがベースキャンプを張る」ような状況となる。そして、温度が高いレプリカが、より低い場所を見つけたら温度を交換する。これにより、エネルギーのより低い場所にベースキャンプを移し、また別の場所を探索に行く。

交換モンテカルロ法は、この温度交換をうまく行うことで、もともとのモンテカルロ法が持っていたサンプリングの重みを変えずに、ローカルミニマムへのトラップの問題を解決できるのだが、僕はこの「ベースキャンプと探検隊」の関係が、数独の難問探索にも応用できるのでは、と考えた。
