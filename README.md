...
TODO: ナイーブなモンテカルロ法の図解

「モンテカルロ法」というと、正方形の中に点をランダムに発生させ、四半円の中に点が入っているかどうかを判定することで円周率を計算する、という「ダーツを投げる」課題を思い出す人が多いかもしれない。これは、$0 < x,y < 1$を満たすような一様乱数$x$と$y$を$m$個発生させ、$x^2 + y^2< 1$となる回数$n$を数えると、その比$n/m$が$\pi/4$になる、ということを使って円周率が求まる、という仕組みだ。一様乱数の発生も、自乗の和も簡単に計算できるわりに、無理数(超越数)である円周率を求めることができるので、モンテカルロ法の例としてよく取り上げられるのだが、この方法は収束が遅いために実用に向かない。

一般に、数値計算手法として使われる「モンテカルロ法」とは、ほぼ「マルコフ連鎖モンテカルロ法(Markov-Chain Monte Carlo Method)」を指す。略してMCMCなどと呼ばれることもある。これはナイーブなモンテカルロ法と違い、極めて収束が早い。特に超多次元空間における数値積分は、事実上この方法しか有効な方法がないため、統計力学において広く使われている。

ナイーブなモンテカルロ法とマルコフ連鎖モンテカルロ法の違いは、「現在の状態」を持つかどうかにある。例えば、
